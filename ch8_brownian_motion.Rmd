...
That the paths are almost surely continuous is trivial. It's also reasonable to assume that the initial value is 0, due to the rules of basketball. However, each quarter does not have 0 mean, which violates one of the Gaussian increment conditions. Nor does it seem that each quarter is iid Normal, as the final quarter has a lower mean than the other quarters. The non-zero mean at each quarter might be an artifact of the data, but 